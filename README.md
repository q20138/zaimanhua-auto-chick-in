...
> **原因说明：**
> 更正：先前将 `ReadingDuration` 视作任务 13 的计时器/进度依据并不严谨。
> 实测存在“任务 13 在 `task/list` 中变为 `status=2`（进行中），但 `ReadingDuration` 仍为 0”的情况，
> 因此不能用 `ReadingDuration` 来判断任务 13 是否计时或是否完成。
>
> 目前能确定的是：网页端阅读可写入阅读位置记录，但在网页抓包中未观察到稳定的“有效阅读时长”上报/心跳机制，
> 因而无法通过网页自动化稳定触发任务 13 的完成。这是平台侧的机制限制，非脚本问题；如需完成此任务，建议使用官方移动端 APP。
